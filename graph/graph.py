...
class Graph:
    ...
    def control_work(self):
        graph = nx.DiGraph()
        graph.add_nodes_from([node.node_id for node in self.nodes])
        graph.add_edges_from([(edge.src_node.node_id, edge.dst_node.node_id) for edge in self.edges])

        result = {'nodes': {src: {} for src in self.nodes}, 'CPL': 0, 'queue_cpl': [], 'queue_cons': []}
        ...
        result['CPL'] = max(value['cpl'] for value in result['nodes'].values())
        result['queue_cpl'] = sorted(result['nodes'], key=lambda i: result['nodes'][i]['cpl'], reverse=True)
        result['queue_cons'] = sorted(result['nodes'], key=lambda i: result['nodes'][i]['cons'], reverse=True)

        for node in result['nodes']:
            print(f'{node.node_id + 1}({node.weight})', result['nodes'][node])
        print()

        print('CPL:', result['CPL'])
        print()

        print(*[f'{node.node_id + 1}({result["nodes"][node]["cpl"]})' for node in result['queue_cpl']], sep=', ')
        print()

        print(*[f'{node.node_id + 1}({result["nodes"][node]["cons"]})' for node in result['queue_cons']], sep=', ')
        print()

        print(sum(node.weight for node in self.nodes))
